    ...
    @if($task->subTasks->count() > 0)
        <div class="mt-4 pt-4 border-t">
            <div class="flex justify-between text-sm text-gray-500 mb-1">
                <span>Progress</span>
                <span>{{ $task->subTasks->where('is_completed', true)->count() }}/{{ $task->subTasks->count() }} selesai</span>
            </div>
        ...
        @forelse($task->subTasks as $sub)
            <div class="flex items-center justify-between gap-3 py-2 border-b last:border-0">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-gray-700 {{ $sub->is_completed ? 'line-through text-gray-400' : '' }}">
                        {{ $sub->title }}
                    </span>
                </div>

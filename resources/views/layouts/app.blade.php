    ...
    <div class="max-w-6xl mx-auto px-6">
        <div class="flex justify-between items-center h-14">
            ...
            <a href="{{ route('tasks.index') }}" class="flex items-center gap-2.5">
                <div class="w-7 h-7 bg-blue-600 rounded-lg flex items-center justify-center text-sm font-bold text-white">✓</div>
                <span class="font-bold tracking-tight text-white" style="font-family:'Syne',sans-serif;">TaskFlow</span>
            </a>
            ...
            <div class="hidden md:flex items-center gap-3">
                ...
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/8">
                    <div class="w-6 h-6 rounded-full bg-blue-900 border border-blue-400 flex items-center justify-center text-blue-300 text-xs font-bold">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="px-3 py-1.5 text-sm text-white/40 hover:text-white border border-white/8 rounded-lg hover:bg-white/5 transition">
                        Logout
                    </button>
                </form>
            </div>
        ...
        <div id="mobile-menu" class="hidden md:hidden pb-4 border-t border-white/5 pt-3 space-y-1">
            <a href="{{ route('dashboard') }}" class="block px-3 py-2 rounded-lg text-sm text-white/60 hover:text-white hover:bg-white/5">Dashboard</a>
            <a href="{{ route('tasks.index') }}" class="block px-3 py-2 rounded-lg text-sm text-white/60 hover:text-white hover:bg-white/5">Tasks</a>
            <a href="{{ route('categories.index') }}" class="block px-3 py-2 rounded-lg text-sm text-white/60 hover:text-white hover:bg-white/5">Kategori</a>
            <div class="flex items-center justify-between pt-3 border-t border-white/5 mt-2">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button class="text-sm text-white/40 border border-white/8 px-3 py-1.5 rounded-lg hover:bg-white/5">Logout</button>
                </form>
            </div>
        </div>
    </div>
...
<div class="max-w-6xl mx-auto px-6 mt-4 space-y-2">
    @if(session('success'))
        <div class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-3 rounded-xl text-sm flex items-center gap-2">
            ✅ {{ session('success') }}
        </div>
    @endif
    @if(session('error'))
